...
public class PsuedoDataSource {
	...
	public static List<Temoignage> createData(int dataSize) {
		if (instance == null)
	...
	public static List<Temoignage> createData() {
		if (instance == null)
			instance = new PsuedoDataSource();
		Temoignage[] thePhotos = new Temoignage[instance.DATA_SIZE];
		for (int i = 0; i < instance.DATA_SIZE; i++) {
			Temoignage photo = new Temoignage(""+idCounter++);
	...
	public static void populate(final List<Temoignage> theList, final ListDataProvider<Temoignage> dataListProvider){
