package fr.nations.amen.client.souvenirs.cellwidget.common;
...
public class PsuedoDataSource {
	...
	public static List<Souvenir> createData(int dataSize) {
		if (instance == null)
	...
	public static List<Souvenir> createData() {
		if (instance == null)
			instance = new PsuedoDataSource();
		Souvenir[] thePhotos = new Souvenir[instance.DATA_SIZE];
		for (int i = 0; i < instance.DATA_SIZE; i++) {
			Souvenir photo = new Souvenir(""+idCounter++);
			...
			photo.setDescription(instance.images[col]);
	...
	public static void populate(final List<Souvenir> theList, final ListDataProvider<Souvenir> dataListProvider){
