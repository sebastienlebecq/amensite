...
public class PsuedoDataSource {
	...
	public static List<SearchVideo> createData(int dataSize) {
		if (instance == null)
	...
	public static List<SearchVideo> createData() {
		if (instance == null)
			instance = new PsuedoDataSource();
		SearchVideo[] thePhotos = new SearchVideo[instance.DATA_SIZE];
		for (int i = 0; i < instance.DATA_SIZE; i++) {
			SearchVideo photo = new SearchVideo(""+idCounter++);
	...
	public static void populate(final List<SearchVideo> theList, final ListDataProvider<SearchVideo> dataListProvider){
