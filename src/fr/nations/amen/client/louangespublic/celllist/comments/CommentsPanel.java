...
public class CommentsPanel extends Composite {
	...
	LouangesServiceAsync souvenirService = GWT
	...
	static class CommentsCell extends AbstractCell<Commentaire> {
		...
		public void render(com.google.gwt.cell.client.Cell.Context context,
			...
			if (value != null) {
				...
				sb.appendHtmlConstant("<table  width='450'><tr>");
					...
					new AsyncCallback<List<Commentaire>>() {
						public void onSuccess(List<Commentaire> result) {
							...
							if (result.size()==0){
							 ...
							 souvenirService.getCountMaxComments(result.get(0).getId(), new AsyncCallback<Integer>() {
									...
									public void onSuccess(Integer countMax) {
										...
										//commentsCountMax = countMax;
										
										updateRowCount(countMax, true);
										updateRowData(start, newData);
										cellList.redraw();
		...
		if (result.size()==0){
		 ...
		 souvenirService.getCountMaxComments(result.get(0).getId(), new AsyncCallback<Integer>() {
				...
				public void onSuccess(Integer countMax) {
					...
					dataProvider.updateRowCount(countMax, true);
					cellList.redraw();
