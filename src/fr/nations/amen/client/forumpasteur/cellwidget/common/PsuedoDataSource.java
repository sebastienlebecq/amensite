...
public class PsuedoDataSource {
	...
	public static List<Messages> createData(int dataSize) {
		if (instance == null)
	...
	public static List<Messages> createData() {
		if (instance == null)
			instance = new PsuedoDataSource();
		Messages[] thePhotos = new Messages[instance.DATA_SIZE];
		for (int i = 0; i < instance.DATA_SIZE; i++) {
			Messages photo = new Messages(""+idCounter++);
	...
	public static void populate(final List<Messages> theList, final ListDataProvider<Messages> dataListProvider){
